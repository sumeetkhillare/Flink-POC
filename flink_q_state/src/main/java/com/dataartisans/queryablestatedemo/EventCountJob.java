...
public class EventCountJob extends RichFlatMapFunction<Tuple2<Long, Long>, Tuple2<String, Long>> {
  ...
  public static void main(String[] args) throws Exception {
    ...
    try {
      ...
      DataStreamSource<String> inp = env.socketTextStream(hostname, 9000, "\n");
              ...
              }).keyBy(0).flatMap(new EventCountJob())
                      .keyBy(0).asQueryableState("average");
  ...
  public void flatMap(Tuple2<Long, Long> input, Collector<Tuple2<String, Long>> out) throws Exception {
    ...
    if (sum.value().f0>=2) {
      out.collect(new Tuple2<>("aaa", (Long.valueOf(sum.value().f1/sum.value().f0))));
    }
  }
