...
target="$base/target/sumjob-queryable-1.0-SNAPSHOT.jar"
if [ ! -f $target ]; then
...
if [ -z $1 ] || [ -z $2 ] || [ -z $3 ] || [ -z $4 ]; then
  ...
  ctime=$3
  parallelism=$4
  /bin/bash $flinkpath run -c com.skhillare.sumjob.flinkjob.StreamingJob $target --port $port --checkpoint $ctime --parallelism $parallelism
  shift
